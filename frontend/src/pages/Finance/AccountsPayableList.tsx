...
import type { BankAccount, CashAccount, PayableDocument, PayableSettlement } from '../../types/finance';
...
export default function AccountsPayableList() {
  ...
  const [drawerDoc, setDrawerDoc] = useState<PayableDocument | null>(null);
  const [payDoc, setPayDoc] = useState<PayableDocument | null>(null);
  ...
  const sourceType = Form.useWatch('source_type', payForm);
  const { config, saveConfig } = useUserPreferences(PAGES.FINANCE_PAYABLES);
  ...
  const listQuery = useQuery({
    ...
    queryFn: () => financeApi.getPayables(params),
  });
  const summaryQuery = useQuery({
    queryKey: ['finance-payables-summary', params],
    queryFn: () => financeApi.getPayableSummary(params),
  });
  const suppliersQuery = useQuery({
    queryKey: ['finance-payable-suppliers'],
    queryFn: () => purchasingApi.getSuppliers({ page_size: 200, ordering: 'code', is_active: 'true' }),
  });
  const detailQuery = useQuery({
    queryKey: ['finance-payable-detail', drawerDoc?.id],
    queryFn: () => financeApi.getPayable(drawerDoc!.id),
    enabled: !!drawerDoc,
  });
  const cashAccountsQuery = useQuery({
    queryKey: ['finance-payable-cash-accounts'],
    queryFn: () => financeApi.getCashAccounts({ page_size: 200, is_active: 'true' }),
  });
  const bankAccountsQuery = useQuery({
    queryKey: ['finance-payable-bank-accounts'],
  ...
  const invalidateAll = async () => {
    await Promise.all([
      ...
      queryClient.invalidateQueries({ queryKey: ['finance-payables-summary'] }),
  ...
  const payMutation = useMutation({
    mutationFn: ({ id, payload }: { id: number; payload: Parameters<typeof financeApi.payPayable>[1] }) =>
      financeApi.payPayable(id, payload),
    onSuccess: async () => {
      await invalidateAll();
      messageApi.success('Đã ghi nhận chi trả');
    },
    onError: (error) => messageApi.error(getToastMessage(error)),
  });
  const cancelMutation = useMutation({
    mutationFn: ({ id, reason }: { id: number; reason: string }) => financeApi.cancelPayable(id, reason),
    onSuccess: async () => {
      await invalidateAll();
      messageApi.success('Đã hủy chứng từ phải trả');
    },
    onError: (error) => messageApi.error(getToastMessage(error)),
  });
  ...
  const suppliers: Supplier[] = suppliersQuery.data?.results ?? [];
  ...
  const columns: ColumnsType<PayableDocument> = [
    { title: 'Mã PP', dataIndex: 'code', width: 130 },
    { title: 'Phiếu nhập', dataIndex: 'source_purchase_receipt_code', width: 130, render: (value) => value || '-' },
    { title: 'Đơn mua', dataIndex: 'source_purchase_order_code', width: 130, render: (value) => value || '-' },
    ...
    { title: 'Đến hạn', dataIndex: 'due_date', width: 120, render: (value) => dayjs(value).format('DD/MM/YYYY') },
    {
      ...
      render: (value) => (
        <Tag color={value === 'OPEN' ? 'warning' : value === 'PARTIAL' ? 'processing' : value === 'SETTLED' ? 'success' : 'default'}>
          {value === 'OPEN' ? 'Chưa chi' : value === 'PARTIAL' ? 'Chi một phần' : value === 'SETTLED' ? 'Đã chi đủ' : 'Đã hủy'}
        </Tag>
      ),
    },
    { title: 'Tổng phải trả', dataIndex: 'total_amount', width: 140, render: (value) => formatMoney(value) },
    { title: 'Đã chi', dataIndex: 'settled_amount', width: 140, render: (value) => formatMoney(value) },
  ...
  const settlementColumns: ColumnsType<PayableSettlement> = [
    { title: 'Ngày chi', dataIndex: 'settlement_date', width: 120, render: (value) => dayjs(value).format('DD/MM/YYYY') },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      {contextHolder}
      <div>
        <h2 style={{ margin: 0 }}>Công nợ phải trả</h2>
        <div style={{ color: '#8c8c8c' }}>Theo dõi chứng từ phải trả từ nhập mua và lịch sử thanh toán</div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 12 }}>
        <Card size="small"><div style={{ color: '#8c8c8c' }}>Tổng phải trả</div><div style={{ fontSize: 24, fontWeight: 700 }}>{formatMoney(summary?.total_amount)}</div></Card>
        <Card size="small"><div style={{ color: '#8c8c8c' }}>Đã chi</div><div style={{ fontSize: 24, fontWeight: 700 }}>{formatMoney(summary?.settled_amount)}</div></Card>
      ...
      <div style={{ border: '1px solid #f0f0f0', borderRadius: 10, padding: 12, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        ...
        <Select
          ...
          options={[
            ...
            { value: 'OPEN', label: 'Chưa chi' },
            { value: 'PARTIAL', label: 'Chi một phần' },
            { value: 'SETTLED', label: 'Đã chi đủ' },
        ...
        <Select
          ...
          options={[
            ...
            ...suppliers.map((item) => ({ value: item.id, label: `${item.code} - ${item.name}` })),
          ]}
        />
        <Space>
        ...
        locale={{
          emptyText: rows.length === 0 && !listQuery.isLoading ? (
            <div style={{ padding: 40, color: '#8c8c8c' }}>
              {(intentSearch || filters.status || filters.supplier || filters.overdueOnly) ? (
                <div>
      ...
      <Modal
        title={payDoc ? `Chi trả ${payDoc.code}` : 'Chi trả'}
        open={!!payDoc}
        onCancel={() => setPayDoc(null)}
        onOk={() => void handlePaySubmit()}
        confirmLoading={payMutation.isPending}
      >
        <Form form={payForm} layout="vertical">
          <Form.Item name="settlement_date" label="Ngày chi" rules={[{ required: true, message: 'Bắt buộc' }]}>
            <Input />
          </Form.Item>
          <Form.Item name="amount" label="Số tiền chi" rules={[{ required: true, message: 'Bắt buộc' }]}>
      ...
      <Drawer
        title={detail ? `Chi tiết ${detail.code}` : 'Chi tiết phải trả'}
        width={920}
        open={!!drawerDoc}
        ...
        {detail ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <Descriptions bordered size="small" column={2}>
              <Descriptions.Item label="Phiếu nhập">{detail.source_purchase_receipt_code || '-'}</Descriptions.Item>
              <Descriptions.Item label="Đơn mua">{detail.source_purchase_order_code || '-'}</Descriptions.Item>
              ...
              <Descriptions.Item label="Trạng thái">
                <Tag color={detail.status === 'OPEN' ? 'warning' : detail.status === 'PARTIAL' ? 'processing' : detail.status === 'SETTLED' ? 'success' : 'default'}>
                  {detail.status === 'OPEN' ? 'Chưa chi' : detail.status === 'PARTIAL' ? 'Chi một phần' : detail.status === 'SETTLED' ? 'Đã chi đủ' : 'Đã hủy'}
                </Tag>
              </Descriptions.Item>
              <Descriptions.Item label="Quá hạn">{`${detail.days_overdue} ngày`}</Descriptions.Item>
              <Descriptions.Item label="Hóa đơn NCC">{detail.vendor_invoice_no || '-'}</Descriptions.Item>
              <Descriptions.Item label="Tổng phải trả">{formatMoney(detail.total_amount)}</Descriptions.Item>
              <Descriptions.Item label="Đã chi">{formatMoney(detail.settled_amount)}</Descriptions.Item>
            ...
            <div>
              <h3 style={{ marginBottom: 8 }}>Lịch sử thanh toán</h3>
              <Table
                rowKey="id"
