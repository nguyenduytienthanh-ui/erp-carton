...
from core.permissions import PURCHASING_PERMISSION_DEFINITIONS


PURCHASING_PERMISSION_KEYS = {
...
def _assert_non_production_target():
    ...
    for value in (app_env, db_name, db_host):
        if _has_production_indicator(value):
            raise CommandError('Refusing Purchasing permission seed on production/prod/live target.')


class Command(BaseCommand):
    help = 'Seed Purchasing view permissions without creating demo users, teams, or business data'

    def add_arguments(self, parser):
        parser.add_argument(
            '--grant-uat-roles',
            action='store_true',
            help='Grant Purchasing view permissions to existing UAT roles using the bootstrap role matrix',
        )
    ...
    def handle(self, *args, **options):
            ...
            if grant_uat_roles:
                for role_row in ROLE_MATRIX:
                    purchasing_keys = [
                        ...
                        if key in PURCHASING_PERMISSION_KEYS
                    ]
                    if not purchasing_keys:
                        continue
                    ...
                    perms = [permission_map[key] for key in purchasing_keys]
                    existing_ids = set(
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'Purchasing permissions synced: {created} created, {updated} updated.'
            )
        )
        if grant_uat_roles:
            self.stdout.write(
                self.style.SUCCESS(
                    f'Purchasing UAT role grants synced: {role_grants} new grants.'
                )
            )
            if skipped_roles:
                self.stdout.write(
